...
test.describe("anonymous user /api/posts api tests", () => {
  let allRolesPostsApiClient: PostsApiClient;
  let anonymousUserPostsApiClient: PostsApiClient;
  ...
  test.beforeEach(async ({ playwright, browser }) => {
    ...
    anonymousUserPostsApiClient = new PostsApiClient(
      await playwright.request.newContext(),
    );
  ...
  test("should find all (published only)", async () => {
    ...
    const findAllPosts = await anonymousUserPostsApiClient.findAll({
      or: [
        {
          id: {
  ...
  test("should find by id (published)", async () => {
    ...
    const foundPost = await anonymousUserPostsApiClient.findById(
      postOnePublished.id,
    );
  ...
  test("should count (published only)", async () => {
    const count = await anonymousUserPostsApiClient.count();
  ...
  test("should not create", async () => {
    ...
    const createdNewPost = await anonymousUserPostsApiClient.create(
      {
        _status: "published",
        title: newPostTitle,
      },
  ...
  test("should not update all", async () => {
    ...
    const updatedPosts = await anonymousUserPostsApiClient.updateAll(
      updateAllQuery,
      {
        title: updatedTitle,
      },
  ...
  test("should not update by id", async () => {
    ...
    const updatedPost = await anonymousUserPostsApiClient.updateById(
      postOnePublished.id,
      {
        title: updatedTitle,
      },
  ...
  test("should not delete all", async () => {
    ...
    const deletedPosts = await anonymousUserPostsApiClient.deleteAll(
      deleteAllQuery,
      "403",
    );

    expect(deletedPosts).toBeNull();
  });
  ...
  test("should not delete by id", async () => {
    ...
    await anonymousUserPostsApiClient.deleteById(postOnePublished.id, "403");
  });
  ...
  test("should not find all versions", async () => {
    ...
    const findAllDocs = await anonymousUserPostsApiClient.findAllVersions(
      {
        or: [
          {
            parent: {
  ...
  test("should not find version by id", async () => {
    ...
    if (findAllDocs && findAllDocs.length > 0) {
      const foundVersion = await anonymousUserPostsApiClient.findVersionById(
        findAllDocs[0].id,
        "403",
      );
  ...
  test("should not restore version by id", async () => {
    ...
    if (findAllDocs && findAllDocs.length > 0) {
      await anonymousUserPostsApiClient.restoreVersionById(
        findAllDocs[0].id,
        "403",
      );
    }
